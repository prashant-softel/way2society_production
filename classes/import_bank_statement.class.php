...
*/
			...
			if($Bank_PaidAmt > 0 && is_numeric($Bank_PaidAmt)) // If Paid Amount is greater than 0 means it's Payment 
			{
				...
				{	
					...
					if($Bank_PaidAmt == $Amount)//  If Amount Match the store data under Match key
					{
						...
						$Result[MATCH_ENTRY][$i]['W2s_Comments'] = str_replace("'"," ",$W2s_PaymentDetails_Comments[$index]);
						$Result[MATCH_ENTRY][$i]['VoucherType'] = $TransactionVoucherType;
						if($IsSplitEntry == true)
						{
							
							$Amount_commnet = implode(',',$SplitEntryValues);
							$Result[MATCH_ENTRY][$i]['error_msg'] = "Cheque Number ".$Bank_ChequeNo." is split entry. Amount ".$Amount_commnet." total ".$Bank_PaidAmt." Matched";
						}
						else
						{
						...
						foreach($tableindexArray as $tableindex) // Remove used data from array
						{
							...
							{
								array_push($LedgerName,str_replace("'"," ",$this->obj_utility->getLedgerName($W2s_PaymentDetails_PaidTo[$tableindex])));	
							}
					...
					else // If Amount Not Matched Store under Present in Bank Key
					{
						...
						{
							$Amount_commnet = implode(',',$SplitEntryValues);
							$Result[PRESENT_IN_BANK][$i]['error_msg'] = "Cheque Number ".$Bank_ChequeNo." is split entry. Amount ".$Amount_commnet." total ".$Amount." not matching";
						}
						else
						{
			...
			else if($Bank_ReceivedAmt > 0 && is_numeric($Bank_ReceivedAmt)) // If Received Amount is greater than 0 means it's Deposit 
			{
				...
				if(!empty($Bank_ChequeNo) && $Bank_ChequeNo <> 0)
				{
					if(in_array($Bank_ChequeNo,$W2s_ChequeEntry_ChequeNo)) // Now we know it's Deposit so we need to check chequeNumber in W2s_ChequeEntry_ChequeNo array
					{
						...
						if($Bank_ReceivedAmt == $Amount) //  If Amount Match the store data under Match key
						{
							...
							$Result[MATCH_ENTRY][$i]['ID'] = $Bank_Register_Ids ;
							$Result[MATCH_ENTRY][$i]['ChkDetailID'] = $Bank_Register_ChkDetailsIds;
							$Result[MATCH_ENTRY][$i]['W2s_Comments'] = str_replace("'"," ",$W2s_ChequeEntry_Comments[$index]);
							$Result[MATCH_ENTRY][$i]['VoucherType'] = $TransactionVoucherType;
							if($IsSplitEntry == true)
							{
								$Amount_commnet = implode(',',$SplitEntryValues);
								$Result[MATCH_ENTRY][$i]['error_msg'] = "Cheque Number ".$Bank_ChequeNo." is split entry. Amount ".$Amount_commnet." total ".$Bank_ReceivedAmt." Matched";
							}
							else
							{
							...
							foreach($tableindexArray as $tableindex)  // Remove used data from array
							{
								...
								{
									array_push($LedgerName,str_replace("'"," ",$this->obj_utility->getLedgerName($W2s_ChequeEntry_PaidBy[$tableindex])));
								}
						...
						else // If Amount Not Matched Store under Present in Bank Key 
						{
								...
								{
									$Amount_commnet = implode(',',$SplitEntryValues);
									$Result[PRESENT_IN_BANK][$i]['error_msg'] = "Cheque Number ".$Bank_ChequeNo." is split entry. Amount ".$Amount_commnet." total ".$Amount." not matching with ".$Bank_ReceivedAmt."";
								}
								else
								{
				...
				else // If We Don't have Cheque Number in Bank Statement
				{
					...
					else
					{
						//echo "<br>Count : ".$i;
						$Result[PRESENT_IN_BANK][$i] = $BankDetails;
						$Result[PRESENT_IN_BANK][$i]['error_msg'] = "ChequeNumber is empty and it is not neft";
					}
				}
			}
			else
					...
					foreach($filter as $index)
					{
						if($Bank_Register_Details[$index]['VoucherTypeID'] == VOUCHER_PAYMENT || $Bank_Register_Details[$index]['VoucherTypeID'] == VOUCHER_CONTRA)
						{
							...
							if(!empty($tableindex))
							{
								...
								if(!empty($W2s_PaymentDetails_PaidTo[$tableindex]))
								{
									$temp_present_in_bank[$AmountMatchCounter]['LedgerName'] = str_replace("'"," ",$this->obj_utility->getLedgerName($W2s_PaymentDetails_PaidTo[$tableindex]));															
								}							
					...
					foreach($filter as $index)
					{
						if($Bank_Register_Details[$index]['VoucherTypeID'] == VOUCHER_RECEIPT || $Bank_Register_Details[$index]['VoucherTypeID'] == VOUCHER_CONTRA)
						{
							...
							if(!empty($tableindex))
							{
								...
								$temp_present_in_bank[$AmountMatchCounter]['LedgerName'] = str_replace("'"," ",$this->obj_utility->getLedgerName($W2s_ChequeEntry_PaidBy[$tableindex]));							
			...
			foreach($Bank_Register_Details as $k => $v)
			{
				...
				if($v['VoucherTypeID'] == VOUCHER_PAYMENT || $v['VoucherTypeID'] == VOUCHER_CONTRA)
				{
					...
					if($index !== false)
					{
						...
						{
							$Result[PRESENT_IN_W2S][$cnt]['LedgerName'] = str_replace("'"," ",$this->obj_utility->getLedgerName($W2s_PaymentDetails_PaidTo[$index]));	
						}
				...
				else if($v['VoucherTypeID'] == VOUCHER_RECEIPT)
				{
					...
					if($index !== false)
					{
						...
						{
							$Result[PRESENT_IN_W2S][$cnt]['LedgerName'] = str_replace("'"," ",$this->obj_utility->getLedgerName($W2s_ChequeEntry_PaidBy[$index]));							
						}
				...
				if(!empty($Comment))
				{
					$Result[PRESENT_IN_W2S][$cnt]['W2s_Comments'] = str_replace("'"," ",$Comment);	
				}
				else
				{
	...
	function getReconcileDetails($Statement_id)
	{
		...
		for($i = 0; $i < count($Bank_Regsiter_Details_Result); $i++)
		{
			...
			if($Bank_Regsiter_Details_Result[$i]['PaidAmount'] > 0)
			{
				...
				$LedgerName = '-';
				$LedgerName = str_replace("'"," ",$this->obj_utility->getLedgerName($Payment_Details[0]['PaidTo']));
			...
			else if($Bank_Regsiter_Details_Result[$i]['ReceivedAmount'] > 0)
			{
				...
				$LedgerName = '-';
				$LedgerName = str_replace("'"," ",$this->obj_utility->getLedgerName($Check_Entry_Details[0]['PaidBy']));
	...
	function ImportActualBankStmt($PresentInBank)
	{
		...
		$this->errorfile_name = '../logs/import_log/'.$Foldername.'/ BankStatement_import_errorlog_'.date("d.m.Y").'_'.rand().'.html';
