			...
			if($row <> '')
			{
				   ...
				   {	
				   		try 
						{
							...
							if($bankIndex === false)
							{
								$errormsg.="<br>Bank &lt;" . $to . "&gt; doesn't exists";
								$BankID = '';
							}
							else
							{
							...
							if(0 < $BillType ||  $BillType > 2 || empty($BillType))
							{
								$errormsg.="<br>Bill Type &lt;" . $BillType . "&gt; doesn't exists";
							}
							if($chequeNo=='')
							{
								$errormsg.="<br>Cheque No Not Provided";
							}
							if($ChequeDate == '' || $ChequeDate== '0' || $ChequeDate=='-' || $ChequeDate=='--')
							{
								$errormsg.="<br>Cheque Date Not Provided";
							}
							else
							{
							...
							if($vNo=='')
							{
								$errormsg.="<br>Voucher Number Not Provided";
							}
							else
							{
								if(!is_numeric($vNo))
								...
								if($unitNo=='')
								{
									$errormsg.="<br>Unit Number Not Provided";
								}
								else
								{
									$unitNo = trim($unitNo);
									...
									{
										$unitNo='';
										$errormsg.="<br>Unit Number Not Found &lt;".$unitNo."&gt;";
									}	
								}
								if($amount=='')
								{
									$errormsg.="<br>Amount Not Provided";
								}
								else
								{
							...
							$successData.="Unit No &lt;".$row[$UnitNo]."&gt; Voucher Number &lt;".$row[$VNo]."&gt; Receipt Date : &lt;".$row[$VDateIndex]."&gt; To_ &lt;".$row[$To]."&gt; Cheque No &lt;".$row[$ChequeNo]."&gt; Cheque Date &lt;".$row[$ChequeDateIndex]."&gt; Payer Bank &lt;".$row[$BankName]."&gt; Payer Branch Name &lt;".$row[$BranchName]."&gt; Amount &lt;".$row[$Amount]."&gt; Account Name &lt;".$row[$AccountName]."&gt; Bill Type &lt;".$row[$BillTypeIndex]."&gt; Remark &lt;".$row[$Remark]."&gt;";
								
							if($vNo!='' && $voucherDate!='' && $chequeNo!='' && $chequeDate!='' && $BankID!='' && $amount!='' && $PaidBy!='' && $BillType!='')
							{
							 ...
							 echo '<br>Voucher Number'.$vNo;
							  $this->obj_ChequeDetails->AddNewValues($voucherDate,$chequeDate,$chequeNo,$vNo,$systemVoucherNo,1,$amount, $PaidBy, $BankID, $PayerBank, $PayerChequeBranch, $DepositeID, $comments,$BillType,0,0,0,0,0,true);
