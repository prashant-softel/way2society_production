...
//							echo "<BR>$sqlSelect<BR>";
					...
					if($BillRegisterID <= 0)
					{
						$sqlInsert = "INSERT INTO `billregister`(`SocietyID`, `PeriodID`, `CreatedBy`, `BillDate`, `DueDate`, `LatestChangeID`, `Notes`,`BillType`,`font_size`) VALUES ('" . $this->m_dbConn->escapeString($result[$k]['SocietyID']). "', '" . $this->m_dbConn->escapeString( $_REQUEST['period_id']). "', '" . $this->m_dbConn->escapeString($_SESSION['login_id']). "', '" . $sBillDate . "', '" . getDBFormatDate($this->m_dbConn->escapeString($_REQUEST['due_date'])) . "', '" . $this->m_dbConn->escapeString($iLatestChangeID). "', '" . $this->m_dbConn->escapeString($Notes). "','".$this->IsSupplementaryBill()."','".$FontSize."')";
...
//$result1 = $this->GetGSTNoThresholdFlag_perMember($societyInfo, $PeriodID, $PrevPeriodID, $PrevPeriodBeginingDate, $PrevPeriodEndingDate,							1);
	...
	public function ReGenerateBill($UnitID, $PeriodID, $IsSupplementaryBill)
	{
		...
		return $BillDetailID = $this->generateBill_PerUnit($UnitID, $PeriodID, $PrevPeriodID, $PrevPeriodBeginingDate, $PrevPeriodEndingDate, $BillRegisterID, $iBillCounter, $sBillDate, $Unit_taxable_no_threshold, $societyInfo);
	}
...
public function ProcessPreviousPrincipal(&$PrevPrincipalDue, &$PaymentReceived, &$CurrentBillInterestAmount, &$PaidPrincipal, $societyInfo, $TransactionType, $PaymentDate, $PrevPeriodBeginingDate, $PrevPeriodEndingDate, $PrevBillDueDate)
{
	...
	if($PaymentReceived > 0) 
	{
		...
		else
		{
			$this->strTrace .=  '<tr><td>No PrevPrincipalDue  to process : ' . $PrevPrincipalDue . '</td></tr>';
				...
				$PrincipalPaymentDateDiff = $this->m_objUtility->getDateDiff($PaymentDate, $PrevBillDueDate);
		...
		if (($TransactionType <> CREDIT_NOTE) && $SubPaidPrincipal>0 && ($PrincipalPaymentDateDiff>0))
		{
			...
			$InterestAmount = $this->GetInterest($UnitID, $SubPaidPrincipal, $PrevPeriodBeginingDate, $PaymentDate, $societyInfo, 1);
	...
	public function ProcessPreviousBillPrincipal(&$PrevBillPrincipalAmount, &$PaymentReceived, &$CurrentBillInterestAmount, &$PaidPrincipal, $societyInfo, $TransactionType, $PaymentDate, $PrevPeriodEndingDate, $PrevBillDueDate)
	{
		$SubPaidPrincipal = 0;			
		...
		if($PaymentReceived > 0) 
		{
			...
			if($PrevBillPrincipalAmount  > 0)
			{
				...
				if (($TransactionType <> CREDIT_NOTE) && $SubPaidPrincipal > 0 && ($PaymentDateDiff  > 0))
				{
					...
					$InterestAmount = $this->GetInterest($UnitID, $SubPaidPrincipal, $PrevBillDueDate, $PaymentDate, $societyInfo);
...
$sqlPrevQuery = "Select bd.ID, BillRegisterID, PrincipalArrears, InterestArrears, `BillSubTotal`, `AdjustmentCredit`, `BillTax`,`IGST`,`CGST`, `SGST`, `CESS`, `BillInterest`, br.BillDate, br.DueDate from billdetails as bd JOIN billregister as br on bd.BillregisterId = br.id where UnitID=" . $UnitID.	 "  and bd.PeriodID=" . $PrevPeriodID." AND bd.BillType ='" . $this->IsSupplementaryBill()."' " ; 
	...
	//					echo '<br/><br/>IsReturn : ' . $IsReturn;
						...
						else
						{
							...
							if($PaymentReceived > 0)
							{
								...
								$this->ProcessPreviousBillPrincipal($PrevBillPrincipalAmount,$PaymentReceived, $CurrentBillInterestAmount, $PaidPrincipal, $societyInfo, $TransactionType, $PaymentDate, $PrevPeriodEndingDate, $PrevBillDueDate);
			...
			if ($PrevBillPrincipalAmount > 0 && $dateDiff < 0)
			{
				...
				else
				{
					...
					if($_SESSION['society_id'] == 247 && $societyInfo['bill_cycle'] == '6' && ($PrevBillDueDate > $PrevPeriodEndingDate))   //For Veena Dalvai : Pending. need to find better solution
					{
						...
						$InterestAmount = $this->GetInterest($UnitID, $InterestableAmount, $PrevBillDueDate, $PrevPeriodEndingDate, $societyInfo);						
					}
					else
					{
						if ($this->ShowDebugTrace == 1)
						...
						//Calculate interest on remaining prev bill amount after the Due date 					
						$InterestAmount = $this->GetInterest($UnitID, $InterestableAmount, $PrevBillDueDate, $PrevPeriodEndingDate, $societyInfo);
					}
			...
			if ($PrevPrincipalDue > 0 && $dateDiff < 0)
			{
				...
				{			
					...
					$InterestAmount = $this->GetInterest($UnitID, $InterestableAmount, $PrevPeriodBeginingDate, $PrevPeriodEndingDate, $societyInfo, 1);
					$CurrentBillInterestAmount = $CurrentBillInterestAmount + $InterestAmount;
			...
			else if($RebateMethod == 3)	//Waive if upto
			{
				//echo '<br/>Rebate Method : Flat';
//				$this->strTrace .=  '<tr><td>Rebate Method : Flat</td></tr>';
				//echo '<br/>Rebate Amount : '. $RebateAmount;
//				$this->strTrace .=  '<tr><td>Rebate Amount : '. $RebateAmount . '</td></tr>';
				...
				$this->strTrace .=  '<tr><td>InterestAmount : ' . $CurrentBillInterestAmount . ' before rebate ' . $RebateAmount . ' removed and made 0</td></tr>';
				$CurrentBillInterestAmount = 0;
				//echo '<br/>InterestAmount after rebate: ' . $CurrentBillInterestAmount;
				//$this->strTrace .=  '<tr><td>InterestAmount after rebate: ' . $CurrentBillInterestAmount . '</td></tr>';
				}
		...
		$LateDays = 0;
		$societyInfo = $this->m_objUtility->GetSocietyInformation($_SESSION['society_id']);
		if($societyInfo['IsRoundOffLedgerAmt'] == 1 && (strtotime($Main_BillDate) >= strtotime('2020-12-01'))){ // If in society table IsRoundOffLedgerAmt is 0 means calculate invoice total by old method 
			
			//echo "<br>Date is greater";
			$BillSubTotal = number_format($BillSubTotal, 2, '.', '');
			$BillTax = number_format($BillTax, 2, '.', '');
			$IGST = number_format($IGST, 2, '.', '');
			$CGST = number_format($CGST, 2, '.', '');
			$SGST = number_format($SGST, 2, '.', '');
			$CESS = number_format($CESS, 2, '.', '');
			$CurrentBillInterestAmount = number_format($CurrentBillInterestAmount, 2, '.', '');
			$PrevPrincipalDue = number_format($PrevPrincipalDue, 2, '.', '');
			$PrevInterestDue = number_format($PrevInterestDue, 2, '.', '');
			
			$Total = $BillSubTotal + $BillTax + $IGST + $CGST + $SGST + $CESS + $CurrentBillInterestAmount + $PrevPrincipalDue + $PrevInterestDue;
...
*/
	public function GetInterest($UnitID, $DueAmount, $DueDate, $PaymentDate, $SQLResult, $bIncludeBothDates = 0)
	{
...
//		$BillSubTotal = -1;
		...
		if($DueAmount > $InterestTrigger)
		{
			...
			if ($InterestMethod == 1)	//DelayAfterDueDate
			{
			...
			else if ($InterestMethod == 2)	//Full Month
			{					
...
//				echo "IsInterestTaxable_NoThreshold". $this->m_IsInterestTaxable_NoThreshold;
	...
	*/
	...
	public function BillDetailsUpdate($Detail,$UnitID,$PeriodID,$CurrentBillInterestAmount,$InterestArrears,$PrincipalArrears,$AdjustmentCredit, $SupplementaryBill)
	{
		...
		$sqlCheck = "select `ID`,`BillInterest`,`PrincipalArrears`,`InterestArrears`,`BillRegisterID`,`LatestChangeID`, `AdjustmentCredit`, `Ledger_round_off` from billdetails where UnitID = '" . $UnitID . "' and PeriodID = '" . $PeriodID . "' and BillType='".$SupplementaryBill ."'" ;
		$resultCheck = $this->m_dbConn->select($sqlCheck);
...
/*
		...
		*/
		...
		if($societyInfo['IsRoundOffLedgerAmt'] == 1 && (strtotime($resultCheck3[0]['BillDate']) >= strtotime('2020-12-01')))
		{
			$BillSubTotal = number_format($BillSubTotal, 2, '.', '');
			$CurrentBillInterestAmount = number_format($CurrentBillInterestAmount, 2, '.', '');
			$AdjustmentCredit = number_format($AdjustmentCredit, 2, '.', '');
			$IGSTAmount = number_format($IGSTAmount, 2, '.', '');
			$CGSTAmount = number_format($CGSTAmount, 2, '.', '');
			$SGSTAmount = number_format($SGSTAmount, 2, '.', '');
			$CESSAmount = number_format($CESSAmount, 2, '.', '');
			$InterestArrears = number_format($InterestArrears, 2, '.', '');
			$PrincipalArrears = number_format($PrincipalArrears, 2, '.', '');
			...
			$RoundOffTotalPayable = $this->m_objUtility->getRoundValue2($TotalBillPayable);
			$RoundOffAmt = $RoundOffTotalPayable - $TotalBillPayable; 
			
			if($RoundOffAmt <> $resultCheck[0]['Ledger_round_off']){
...
*/
	...
	public function deleteInvoice($InvoiceID, $unitID, $billdate, $IsRequestEditInvoice ,$bSkipBeginTrnx=false )
	{
		...
		if($IsRequestEditInvoice == 1)
		{
			...
			$iLatestChangeID = $this->m_objLog ->setLog($desc, $_SESSION['login_id'], 'sale_invoice', $InvoiceID);		
		}
	...
	public function ShowTaxable($LedgerID)
	{
		 ...
		 $res = $this->$dbConn->select($checktaxableLedger);
	...
	public function AddCreditDebitNoteWithImport($Detail,$UnitID,$bill_date,$BillType,$NoteType,$BillNote,$IseditModeSet,$CreditDebitEditable_ID,$IsCallUpdtCnt = 1,$ExternalCounter, $flag = 0,$GUID,$IsRequestFromImportCredit,$CalculatedGSTDetails)
	{
		...
		else
		{
			...
			$iLatestChangeID = $this->m_objLog ->setLog($desc, $_SESSION['login_id'], 'credit_debit_note', $RefNo);
