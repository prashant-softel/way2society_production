...
//$m_TraceDebugInfo = "<br><br>Got the columns to be updated!... Column Names are: ". $BCode. " ". $WCode. " " .$FCode. " ".$FloorNo. " " . $Owner. " ". $Occupation. " " . $DateOfBirth . " " . $AnnivarsaryDate. " ". $Gender . " " .$BloodGroup. " " . $OwnerAddress. " " . $BikeParkingNo. " " . $CarParkingNo . " ".$FlatArea." " . $GSTINNO. " " . $ResPhone. " " .$MobileNo . " " . $OffPhone. " " . $EMail. " " . $Email1. " " . $DisposeDate. " " . $Inactive. " " .$EmergencyMobileNo. " " . $EmergencyPersonName. " " . $EmergencyTelephoneNo."   ".$Parking_No. "  ".$FlatConfiguration." ".$ExtnLedgerNameIndex."";
	...
	// $this->obj_utility->logGenerator($errorfile,$rowCount,$m_TraceDebugInfo,"I");
		...
		if($owner_name != '')
			...
			if ($associatemembername != '') 
			{
				$m_TraceDebugInfo .= "associate member name : &lt;" .$associatemembername . "&gt;";
				//$this->obj_utility->logGenerator($errorfile,$rowCount,$m_TraceDebugInfo,"I");
			}
			if ($virtualac != '') 
			{
			...
			if(array_search($AssociateMemberName,$indexes))
                     {
	
				if (isset($AssociateMemberName))
				{
				   if ($associatemembername != '')
					{
						$get_member_id = "select `member_id` FROM `member_main` where unit='".$unit."'";
						...
						{	
						  ...
						  $sql2 = "insert into `mem_other_family` (`member_id`, `other_name`,`relation`,`coowner`) VALUES ('" . $member_id . "', '".$associatemembername."','Self','3')";
						
						 $resCoOwner =$this->m_dbConn->insert($sql2);
						$m_TraceDebugInfo.= "Updated associate member where Flat No: <" .$unit_no. "><br>";
					}
				else{
					  $m_TraceDebugInfo.="Invalid associate member &lt;".$associatemembername."&gt;";
			...
			if(array_search($VirtualAC,$indexes))
            {
				...
				if (isset($VirtualAC))
				{
				   if ($virtualac != '')
					{
						$sql1 = "UPDATE unit SET `virtual_acc_no` ='".$virtualac."' WHERE unit_id='".$unit."'";
						$this->m_dbConn->update($sql1);
						$m_TraceDebugInfo.= "Updated virtual account number where Flat No: <" .$unit_no. "><br>";
					}
				else{
					  $m_TraceDebugInfo.="Invalid virtual account number &lt;".$virtualac."&gt;";
			...
			if(array_search($MobileNo,$indexes))
            {
				if (isset($MobileNo)) 
				{
					...
					if ($mob != '')
					{
						$sql1 = "UPDATE member_main set `mob`='".$mob."' WHERE unit='".$unit."'";
						$this->m_dbConn->update($sql1);
					
						$m_TraceDebugInfo.= "Updated mob where Flat No: <" .$unit_no. "><br>";
			...
			if(array_search($OffPhone,$indexes))
            {
				if (isset($OffPhone)) 
				{
					...
					if ($off_no != '')
					{
						$sql1 = "UPDATE member_main set `off_no`='".$off_no."' WHERE unit='".$unit."'";
						$this->m_dbConn->update($sql1);
				...
				if (isset($EMail))
				{
					...
					if ($email != '')
					{
						...
						{		
							$sql1 = "UPDATE member_main set `email`='".$email."' WHERE unit='".$unit."'";
						   $this->m_dbConn->update($sql1);
					
						    $m_TraceDebugInfo.= "Updated email where Flat No: <" .$unit_no. "><br>";
				...
				if (isset($Email1))
				{
					...
					if ($alt_email != '')
					{
						...
						{
							$sql1 = "UPDATE member_main set `alt_email`='".$alt_email."' WHERE unit='".$unit."'";
						    $this->m_dbConn->update($sql1);
					
						   $m_TraceDebugInfo.= "Updated alt email where Flat No: <" .$unit_no. "><br>";
						}
					}
				}
		...
		/*	if(array_search($DisposeDate,$indexes))
			...
			if(array_search($EmergencyPersonName,$indexes))
            {
				if (isset($EmergencyPersonName))
				{
					...
					if ($eme_rel_name != '')
					{ 
						$sql1 = "UPDATE member_main set `eme_rel_name`='".$eme_rel_name."' WHERE unit='".$unit."'";
						$this->m_dbConn->update($sql1);
					
						$m_TraceDebugInfo.= "Updated eme name where Flat No: <" .$unit_no. "><br>";
