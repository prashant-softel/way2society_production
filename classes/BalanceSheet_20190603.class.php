...
class BalanceSheet
{
	...
	function __construct($dbConn)
	{
		...
		$this->IE_ledgerDetails = $this->m_objUtility->getParentOfLedger($_SESSION['default_income_expenditure_account']);
		
		//echo "<BR>IE category " . $this->IE_ledgerDetails['category'] ; 
		//var_dump($this->IE_ledgerDetails);
	...
	public function getIncomeExpenditureledgerDetails($LedgerID)
	{
		$result = array();
		
		$LedgerOpeningBalance = $this->m_objUtility->getOpeningBalance($LedgerID,$_SESSION['default_year_start_date']);
		$ledgerDetails = $this->m_objUtility->getParentOfLedger($LedgerID);
		$voucherdetailsquery = "SELECT * FROM `voucher` where Date BETWEEN '".$_SESSION['default_year_start_date']."' AND '".$_SESSION['default_year_end_date']."' AND (`By` = '".$LedgerID."' or `To` = '".$LedgerID . "')" ;
		$voucherDetails = $this->m_dbConn->select($voucherdetailsquery);
		
		$cnt = 0;
		$result[$cnt]['CategoryID'] = $ledgerDetails['category'];
		
	
		if($voucherDetails == NULL)
		{
			$result[$cnt]['AddorLess'] = "Add";
			$result[$cnt]['LedgerName'] =  $this->m_objUtility->getLedgerName($_SESSION['default_income_expenditure_account']);
			$result[$cnt]['OpeningBalance'] = $LedgerOpeningBalance['Credit'] - $LedgerOpeningBalance['Debit'];
			$result[$cnt]['Amt'] = 0;
			if($result[$cnt]['OpeningBalance'] < 0)
			{
				$result[$cnt]['AddorLess'] = "Less";
			}
		}
		else
		{
			$result[$cnt]['OpeningBalance'] = $LedgerOpeningBalance['Total']; 
			$ExpenditureDetailsQuery = "SELECT * FROM `voucher` where VoucherNo = '".$voucherDetails[0]['VoucherNo']."'";
			$ExpenditureDetails = $this->m_dbConn->select($ExpenditureDetailsQuery);
			
			
			for($i = 0 ; $i < sizeof($ExpenditureDetails); $i++)
			{
				if($ExpenditureDetails[$i]['To'] == '' && $ExpenditureDetails[$i]['By'] <> $LedgerID)
				{
					$result[$cnt]['LedgerName'] = $this->m_objUtility->getLedgerName($ExpenditureDetails[$i]['By']);
					$result[$cnt]['Amt'] = $ExpenditureDetails[$i]['Debit'];
					$result[$cnt]['AddorLess'] = "Add";
				}
				else if($ExpenditureDetails[$i]['By'] == '' && $ExpenditureDetails[$i]['To'] <> $LedgerID)
				{
					$result[$cnt]['LedgerName'] = $this->m_objUtility->getLedgerName($ExpenditureDetails[$i]['To']);
					$result[$cnt]['Amt'] = -1*$ExpenditureDetails[$i]['Credit'];
					$result[$cnt]['AddorLess'] = "Less";
				}
	...
	public function generateBalanceSheet2($datas, $parent = 0,$type, $depth = 0)
	{
		...
		foreach($datas as $key)
		{
			...
			if($key['parent_id'] == $parent)
			{
				...
				if($key['LedgerID'] == $_SESSION['default_income_expenditure_account'])
				{	
					continue;
				}
				//$amount = abs($amount);
				...
				$tree .= $this->generateBalanceSheet2($datas, $key['id'],$type,$depth+1);
				$tree .= '</li>';
			}
		}
				...
				else
				{
					...
					if($datas[$i]['LedgerID'] > 0)
					{
						...
						if(!isset($_GET['q']) || (isset($_GET['q']) && $_GET['q'] == 2))
						{
							...
							$tree .= '<a href = "'.$Url.'" target = "_blank">'.$datas[$i]['name'].'</a></div><div style="border:none;width:12.5%;text-align:right;float:left;" class="icon-minus-sign"><a href = "'.$Url.'" target = "_blank">'.number_format($datas[$i]['OpeningBalanceCredit'],2).'</a></div>
						...
						else 
						{
							//summary tree view i.e q=1
									if(isset($_GET['show']) &&  $_GET['show'] == 1)
									{
											...
											$tree .= '<a href = "'.$Url.'" target = "_blank">'.$datas[$i]['name'].'</a></div><div style="border:none;width:17.5%;text-align:right;float:left;display:none" class="icon-minus-sign"><a href="'.$Url.'" target = "_blank">'.number_format($datas[$i]['credit'],2).'</a></div>
...
/*	public function generateIncomeStatement($GroupID,$datas, $parent = 0, $depth = 0, $padding = 10)
	...
	public function CalculateTotal($categories)
	{
		...
		for($iCnt = sizeof($categories)-1; $iCnt >= 0 ; $iCnt--)
		{
			if($categories[$iCnt]['LedgerID'] == $_SESSION['default_income_expenditure_account'])
			{
				continue;
			}
			$parent = $categories[$iCnt]['parent_id'];
	...
	public function ArrayShifting($GroupID,&$Data,&$DataShiftarray)
	{
		...
		for($i = 0; $i < sizeof($Data); $i++)
		{
			...
			if( ($Data[$i]['parent_id'] == 0 || $Data[$i]['parent_id'] == 1))
			{
				...
				{
					if($Data[$i]['id'] == $this->IE_ledgerDetails['category'])
					{
						continue;
					}	
