	...
	public function FetchAllNotices($nid=0, $UnitID = 0)
	{
		...
		if($_SESSION['role'] && ($_SESSION['role']==ROLE_ADMIN || $_SESSION['role']==ROLE_SUPER_ADMIN || $_SESSION['role']==ROLE_ADMIN_MEMBER || $_SESSION['role']==ROLE_MANAGER))
		{
			...
			if($nid <> 0)
			{
			 ...
			 $sql ="select noticetbl.* FROM notices noticetbl WHERE noticetbl.society_id=".$_SESSION['society_id']." and noticetbl.status = 'Y' and noticetbl.id=".$nid."  ORDER BY noticetbl.exp_date DESC";	
			  ...
			  else
			  {
		  		...
		  		if($bSendMail == true)
				{
					...
					if($EMailIDToUse['status'] == 0)				
					{
						...
						//if(($emailCount >= 11) && ($emailCount < 17))
