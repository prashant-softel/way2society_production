	...
	public function generateBalanceSheet_withPreviousyear($datas, $parent = 0,$type, $depth = 0)
	{
		...
		foreach($datas as $key)
		{
			...
			if($key['parent_id'] == $parent)
			{
				...
				if(($PreviousYearamount > 0) && $type == '2')
				{
					$amountPrevType = ' Dr';
				}
				elseif(($PreviousYearamount < 0)&& $type == '2')
				{
					$amountPrevType = ' Cr';	

				}
				elseif(($PreviousYearamount > 0) && $type == '1')
				{
					$amountPrevType = ' Cr';	
				}
				elseif(($PreviousYearamount < 0) && $type == '1')
				...
				if($amount <> 0 || $PreviousYearamount <> 0)
				{
					...
					else
					{
						...
						{
							$Url = "view_ledger_details.php?lid=".$key['LedgerID']."&gid=".$key['GroupType']."&dt";
							$tree .= '<a href = "'.$Url.'&prev" target="_blank">'.number_format(abs($PreviousYearamount),2). $amountPrevType.'</a></span><span style=" border:none;width:180px;" class="icon-minus-sign"><a href = "'.$LedgetUrl.'" target="_blank">'. $key['name']. '</a></span><span style="border:none;width:100px;text-align:right;" class="icon-minus-sign"><a href = "'.$Url.'" target="_blank">'.number_format(abs($amount),2). $amountType.'</a></span></label>';
						}
						else 
						{
							$tree .= number_format(abs($PreviousYearamount),2). $amountPrevType.'</span><span style=" border:none;width:180px;" class="icon-minus-sign">'.$key['name'].'</span><span style="border:none;width:100px;text-align:right;" class="icon-minus-sign">'.number_format(abs($amount),2). $amountType.'</span></label>';
						}
