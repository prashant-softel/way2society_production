<?php 
	...
	function Send_invitaion_to_all_member()
	{
		...
		$mailer = getTransport($obj_Utility, $getTransport);
		...
		for($EmailCount = 0 ; $i < count($emailIDList); $i++)
		{
			if($debug_trace == 1)
			{
			...
			//if(!in_array($mailToEmail,$Invition_Sent_EmailID_Lists)) // Checking whether email already not sent to this email id. 
				...
				if($result >= 1)
				{
					...
					if(($success % 49) == 0)
					{
						echo "<br>New Transport Object Created";
						
						$function_count++;
						$mailer = getTransport($obj_Utility,$function_count);
					}
