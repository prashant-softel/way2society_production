...
ini_set('memory_limit', '2048M');  // exceed memory limit
...
class notice
{
	...
	public function sendEmail($subject, $desc, $noticeToArray, $fileName, $NoticeID, $UnitID, $SocID, $DBName, $bCronjobProcess, $QueueID)
	{
		...
		$mailSubject = $subject ;
		$mailBody = $desc;
		//echo "<br/>Notice to display:". sizeof($noticeToArray) . "  batch size " . $NoticeBatchSize."<br>";														
		$display = array();
	  ...
	  try
	  {
		  ...
		  //echo "<br/>Star DIspaly";
		  //var_dump($display);
		  //echo "<br>End Of Display";
		  foreach($display as $key => $value)
		  {
			  ...
			  else
			  {
		  		...
		  		if($bSendMail == true)
				{
					...
					//echo "<br> Status :".$EMailIDToUse['status'];
					//die();
					if($EMailIDToUse['status'] == 0)				
					{
						...
						$AWS_Config = CommanEmailConfig();
						 $transport = Swift_SmtpTransport::newInstance($AWS_Config[0]['Endpoint'],$AWS_Config[0]['Port'] , $AWS_Config[0]['Security'])
							...
							$message->setCc('accounts@way2society.com');
							...
							//echo "<br>Send ".$sendind;
						//}
						
						$emailCount ++;
						
						//echo "<BR>Batch $emailCount Result ". $result;
						//var_dunp($bccUnitsArray);
						foreach($bccUnitsArray as $key => $value)
		  				{
								//echo "<br/>Inside BCC Unit Success";
