...
class BalanceSheet
{
	...
	public function setAmount($datas, $parent, $creditamount,$debitamount, $maxId,$openingBalanceCredit,$openingBalanceDebit,$closingBalanceCredit,$closingBalanceDebit)
	{
		$total=0;
	...
	public function generateTrialBalance($datas, $parent = 0, $depth=0, $padding = 10)
	{
		...
		//$tree = '<ul style="margin-right: -26px; margin-left: -11px;">';
		$tree = '<ul >';
		for($i = 0, $ni = count($datas); $i < $ni; $i++){
			if($datas[$i]['parent_id'] == $parent && ($datas[$i]['credit'] > 0 OR  $datas[$i]['debit'] >0)){
				if($parent == 0 OR $parent == 1)
				{
					...
					$tree .= '<li id="li_' . $datas[$i]['id'] .'_'.$depth.  '" style="border:none;"><label style="width:100%;"><div style="width:100%;border:none;"><div style="border:none;width:24%; float:left;" >';	
					...
					{
						$tree .= $datas[$i]['name'].'</a></div><div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($datas[$i]['credit'],2).'</div>
																<div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($datas[$i]['debit'],2).'</div>
																<div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($closingBalanceCredit,2).'</div>
																<div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($closingBalanceDebit,2).'</div></div></label>';	
					}
					$balance = 0;
				}
				else
				{
					$tree .= '<li id="li_' . $datas[$i]['id'] .'_'.$depth.  '"  style="display:none;"><label style="width:100%;"><div style="width:100%;border:none;"><div style=" border:none;width:24%;float:left;" class="icon-minus-sign">';	
					...
					if($datas[$i]['LedgerID'] > 0)
					{
						...
						{
							$tree .= '<a href="'.$Url.'">'.$datas[$i]['name'].'</a></div><div style="border:none;width:17.5%;text-align:right;float:left;" class="icon-minus-sign"><a href="'.$Url.'">'.number_format($datas[$i]['credit'],2).'</a></div>
																					<div style="border:none;width:17.5%;text-align:right;float:left;"><a href="'.$Url.'">'.number_format($datas[$i]['debit'],2).'</a></div>
																					<div style="border:none;width:17.5%;text-align:right;float:left;"><a href="'.$Url.'">'.number_format($datas[$i]['ClosingBalanceCredit'],2).'</a></div>
																					<div style="border:none;width:17.5%;text-align:right;float:left;"><a href="'.$Url.'">'.number_format($datas[$i]['ClosingBalanceDebit'],2).'</a></div></label>';		
						}
					}
					else
					{
						...
						{
							$tree .= $datas[$i]['name'].'</div><div style="border:none;width:17.5%;text-align:right;float:left;" class="icon-minus-sign">'.number_format($datas[$i]['credit'],2).'</div>
															<div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($datas[$i]['debit'],2).'</div>
															<div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($datas[$i]['ClosingBalanceCredit'],2).'</div>
															<div style="border:none;width:17.5%;text-align:right;float:left;">'.number_format($datas[$i]['ClosingBalanceDebit'],2).'</div></div></label>';	
		...
		for($i = 0, $ni = count($datas); $i < $ni; $i++){
			if($datas[$i]['parent_id'] == $parent && ($datas[$i]['credit'] > 0 OR  $datas[$i]['debit'] >0)){
				if($parent == 0 OR $parent == 1)
				{
					...
					$tree .= $datas[$i]['name'].'</b></span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['OpeningBalanceCredit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['OpeningBalanceDebit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['credit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['debit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($closingBalanceCredit,2).'</span></td><td style="text-align:right;"><span >'.number_format($closingBalanceDebit,2).'</span></td></label>';
					$balance = 0;
				}
				else
				{
					...
					{
						$Url = "view_ledger_details.php?lid=".$datas[$i]['LedgerID']."&gid=".$datas[$i]['GroupType']."&dt";
						$tree .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.$Url.'">'.$datas[$i]['name'].'</a></span></td><td style="text-align:right;"><span ><a href="'.$Url.'">'.number_format($datas[$i]['OpeningBalanceCredit'],2).'</a></span></td><td style="text-align:right;"><span ><a href="'.$Url.'">'.number_format($datas[$i]['OpeningBalanceDebit'],2).'</a></span></td><td style="text-align:right;" ><span ><a href="'.$Url.'">'.number_format($datas[$i]['credit'],2).'</a></span></td><td style="text-align:right;"><span ><a href="'.$Url.'">'.number_format($datas[$i]['debit'],2).'</a></span></td><td style="text-align:right;"><span ><a href="'.$Url.'">'.number_format($datas[$i]['ClosingBalanceCredit'],2).'</a></span></td><td style="text-align:right;"><span ><a href="'.$Url.'">'.number_format($datas[$i]['ClosingBalanceDebit'],2).'</a></span></td></label>';
					}
					else
					{
						$tree .= '<b>&nbsp;&nbsp;&nbsp;'.$datas[$i]['name'].'</b></span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['OpeningBalanceCredit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['OpeningBalanceDebit'],2).'</span></td><td style="text-align:right;" ><span >'.number_format($datas[$i]['credit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['debit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['ClosingBalanceCredit'],2).'</span></td><td style="text-align:right;"><span >'.number_format($datas[$i]['ClosingBalanceDebit'],2).'</span></td></label>';
					}
	...
	public function FormatArray($Data,$GroupID, $from, $to,$IncomeExpenseAccount=0)
	{
		...
		foreach($Data as $category) {
			...
			$categories[] = array('id' => $category['category_id'], 'name' =>$category['category_name'], 'parent_id' => $category['parentcategory_id'], 'credit' =>0,'IsParent' => $parent,'debit' =>0,'GroupType' => $type,'LedgerID' => 0,'OpeningBalanceCredit' => 0,'OpeningBalanceDebit' => 0,'ClosingBalanceCredit' => 0,'ClosingBalanceDebit' => 0);
			...
			foreach($LedgerData as $category2) 
			{
				...
				if($GroupID == 1 || $GroupID == 2)
				{
					...
					{
						$resdata = explode('_',$openingLedgerBalance); 
						if($resdata <> "")
						{
							if($resdata[0] == 'Cr')
							{
								$openingLedgerBalanceCredit = $resdata[1];	
							}
							else if($resdata[0] == 'Dr')
							{
								$openingLedgerBalanceDebit = $resdata[1];	
							}
						}
					}
				}
	...
	public function CalculateTotal($categories)
	{
		for($iCnt = sizeof($categories)-1; $iCnt >=0 ; $iCnt--)
		{
			...
			$categories = $this->setAmount($categories, $parent, $categories[$iCnt]['credit'],$categories[$iCnt]['debit'], $iCnt,$categories[$iCnt]['OpeningBalanceCredit'],$categories[$iCnt]['OpeningBalanceDebit'],$categories[$iCnt]['ClosingBalanceCredit'],$categories[$iCnt]['ClosingBalanceDebit']);
		}
		for($iCnt = sizeof($categories)-1; $iCnt >= 0 ;$iCnt--)
		{
	...
	public function getLedgerOpeningBalance($lid,$gid)
	{
		...
		if($tableName <> "")
		{
			...
			if($res[0]['Credit'] > 0)
			{
				return 	'Cr_'.$res[0]['Credit'];
			}
			else if($res[0]['Debit'] > 0)
			{
				return 	'Dr_'.$res[0]['Debit'];	
			}
			else
			{
				return 'None_0';	
