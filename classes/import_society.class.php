			...
			foreach  ($fileData as $row)
			{
				...
				if($row[0] || $row[1] || $row[2]<> '')
				{
						...
						if($rowCount == 1)//Header
						{
							...
							$IntercomIndex=array_search(IntercomNo,$row,true);
					   	...
					   	{   
							...
							for($i=0;$i<sizeof($row);$i++)
							{
								...
								if(in_array($i,$checkbox_indexes))
								{
									...
									{
										$intercom_number='';
										$intercom_number =$row[$IntercomIndex];
										
										if($intercom_number == '')
										{
											$ErrorMsg .="<br>Intercome Number not provided in line :".$rowCount;
										}
										else
										{
								
											$intercom_no = $intercom_number;
										}
											
										
										   $successData .="Intercom member : &lt;".$intercom_no."&gt;";
										   continue;
									}
							...
							else if(($society_id <> "" || $society_id <> 0) && ($wing_id <> "" || $wing_id <> 0))
							{
								...
								$insert_unit="insert into unit(unit_id,society_id,wing_id,unit_no,floor_no,unit_type,area,sort_order,flat_configuration,virtual_acc_no,intercom_no) values('".$data4."','$society_id','$wing_id','$unit_no','$floor_no','$unit_type','$area',".$SortOrderID.",'$flat_configuration','$virtualac','$intercom_no')";
								$data=$this->m_dbConn->insert($insert_unit);
								$isImportSuccessUnit = true;
