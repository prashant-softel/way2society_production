				...
				{														
					...
					{							
						...
						{												
							...
							//Reciept ENtry 
							if($details[$i]['VoucherTypeID'] == VOUCHER_RECEIPT)
							{
							...
							} // end if in Cheque return
							// Payment Return 	 
							else if($details[$i]['VoucherTypeID'] == VOUCHER_PAYMENT)
							{
								$selectQuery = 'SELECT `id` FROM `depositgroup` WHERE  `BankID` = "'.$_POST["ledgerID"].'" AND `DepositSlipCreatedYearID` = "'.$_SESSION['default_year'].'"';
								...
								{	
									...
									$insert_query="insert into depositgroup (`bankid`,`createby`,`depositedby`,`desc`, `DepositSlipCreatedYearID`) values (".$_POST["ledgerID"]."','".$_SESSION['login_id']."','ReversePayment-".$yearDesc."','Deposite Created for bounce cheque entries : ".$yearDesc.".', '".$_SESSION['default_year']."')";
									$depositeID = $this->m_dbConn->insert($insert_query);
								}
								else
								{
									$depositeID = $result[0]['id'];	
								}

								$BillType = $this->getBillType($details[$i]['ChkDetailID']);
								
								$RecievedAmount = $details[$i]['PaidAmount'];
								$comment = 'Reverse entry for Payment #'.$details[$i]['ChequeNumber'].' from '.$details[$i]['ledger_name'].'.';
								
								$insert_query="insert into chequeentrydetails(`ChequeDate`, `ChequeNumber`, `Amount`, `PaidBy`, `EnteredBy`, `PayerBank`, `Comments`, `VoucherDate`,  `BankID`,`DepositID`) values ('".getDBFormatDate($_POST['cancel_date'.$i])."', '".$details[$i]['ChequeNumber']."', '".$_POST['bank_penalty']."', '".BANK_CHARGES."', '".$_SESSION['login_id']."', '".$_POST['ledgerID']."', '".$comment."','".getDBFormatDate($_POST['cancel_date'.$i])."', '0','".$depositeID."')";												
								$data = $this->m_dbConn->insert($insert_query);
								
								$insert_query="insert into chequeentrydetails(`ChequeDate`, `ChequeNumber`, `Amount`, `PaidBy`, `EnteredBy`, `PayerBank`, `Comments`, `VoucherDate`,  `BankID`, `BillType`,`DepositID`) values ('".getDBFormatDate($_POST['cancel_date'.$i])."', '".$details[$i]['ChequeNumber']."', '".$RecievedAmount."', '".$details[$i]['id']."', '".$_SESSION['login_id']."', '".$_POST['ledgerID']."', '".$comment."','".getDBFormatDate($_POST['cancel_date'.$i])."',  '".$details[$i]['ChkDetailID']."', '".$BillType."','".$depositeID."')";						
								$data1 = $this->m_dbConn->insert($insert_query);
								
								$note = 'Cheque #'.$details[$i]['ChequeNumber'].' from '.$details[$i]['ledger_name'].' is rejected. ['.$_POST['note'.$i].']';
								$amount =$RecievedAmount + $_POST['bank_penalty'];	
								//$total_penalty = $_POST['bank_penalty'] + $_POST['society_penalty'];
								$iVoucherCounter = $obj_LatestCount->getLatestVoucherNo($_SESSION['society_id']);	
								//echo $iVoucherCounter;
								
								$voucherID_I = $obj_voucher->SetVoucherDetails(getDBFormatDate($_POST['cancel_date'.$i]),$data, TABLE_CHEQUE_DETAILS, $iVoucherCounter,$iSrNo, VOUCHER_RECEIPT,$_POST['ledgerID'],TRANSACTION_DEBIT,$_POST['bank_penalty'], $note);																	
								
								$voucherID_II = $obj_voucher->SetVoucherDetails(getDBFormatDate($_POST['cancel_date'.$i]),$data, TABLE_CHEQUE_DETAILS, $iVoucherCounter,$iSrNo+1, VOUCHER_RECEIPT,BANK_CHARGES,TRANSACTION_CREDIT,$_POST['bank_penalty'], $note);
								
								//$society_amount = $_POST['amount'.$i] + $_POST['society_penalty'];
								$iVoucherCounter = $obj_LatestCount->getLatestVoucherNo($_SESSION['society_id']);
								
								$voucherID_III = $obj_voucher->SetVoucherDetails(getDBFormatDate($_POST['cancel_date'.$i]),$data1,TABLE_CHEQUE_DETAILS,$iVoucherCounter,$iSrNo,VOUCHER_RECEIPT,$_POST['ledgerID'],TRANSACTION_DEBIT,$RecievedAmount,$note);				
								
								$voucherID_IV = $obj_voucher->SetVoucherDetails(getDBFormatDate($_POST['cancel_date'.$i]),$data1,TABLE_CHEQUE_DETAILS, $iVoucherCounter,$iSrNo+1, VOUCHER_RECEIPT,$details[$i]['id'],TRANSACTION_CREDIT,$RecievedAmount, $note);						
		
								$obj_register->SetBankRegister($_POST['cancel_date'.$i],$_POST['ledgerID'],$voucherID_III,VOUCHER_RECEIPT, TRANSACTION_RECEIVED_AMOUNT,$RecievedAmount,$chqleafID,$data1,0,$details[$i]['Cheque Date'], $details[$i]['ID']);						
							
								$obj_register->SetBankRegister($_POST['cancel_date'.$i],$_POST['ledgerID'],$voucherID_I,VOUCHER_RECEIPT,TRANSACTION_RECEIVED_AMOUNT,$_POST['bank_penalty'],$chqleafID,$data,0,$details[$i]['Cheque Date'],$details[$i]['ID']);						
							
								//$obj_register->SetIncomeRegister(PENALTY_TO_MEMBER, $_POST['cancel_date'.$i], 0, "", TRANSACTION_CREDIT, $_POST['society_penalty']);
								$obj_register->SetExpenseRegister(BANK_CHARGES, $_POST['cancel_date'.$i], $voucherID_II, VOUCHER_RECEIPT, TRANSACTION_CREDIT, $_POST['bank_penalty'], 0); 																		
								...
								if(!(empty($arParentDetails)))
								{
									...
									if($ExpenseByGroupID==LIABILITY)
									{																						
										$obj_register->SetLiabilityRegister(getDBFormatDate($_POST['cancel_date'.$i]),$details[$i]['id'],$voucherID_IV,VOUCHER_RECEIPT, TRANSACTION_CREDIT, $RecievedAmount, 0);													
									}								
									else if($ExpenseByGroupID==ASSET)
									{
										if($PaidToCategoryID == BANK_ACCOUNT || $PaidToCategoryID == CASH_ACCOUNT)						
										{
											$obj_register->SetBankRegister(getDBFormatDate($_POST['cancel_date'.$i]), $details[$i]['id'], $voucherID_IV, VOUCHER_RECEIPT, TRANSACTION_PAID_AMOUNT, $RecievedAmount, $chqleafID, $data1, 0, getDBFormatDate($details[$i]['Cheque Date']), $details[$i]['ID']);
										}
										else
										{
											$obj_register->SetAssetRegister(getDBFormatDate($_POST['cancel_date'.$i]), $details[$i]['id'], $voucherID_IV, VOUCHER_RECEIPT, TRANSACTION_CREDIT, $RecievedAmount, 0);				
										}
									}								
									else if($ExpenseByGroupID==INCOME)
									{													
										$obj_register->SetIncomeRegister($details[$i]['id'], getDBFormatDate($_POST['cancel_date'.$i]), $voucherID_IV, VOUCHER_RECEIPT, TRANSACTION_CREDIT, $RecievedAmount);
									}								
									else if($ExpenseByGroupID==EXPENSE)
									{																		 
										$obj_register->SetExpenseRegister($details[$i]['id'],getDBFormatDate($_POST['cancel_date'.$i]), $voucherID_IV,VOUCHER_RECEIPT, TRANSACTION_CREDIT,$RecievedAmount,0);												
									}						
								}
								...
								if($details[$i]['ledger_name'] != "Opening Balance")
								{						
									$update_paymentDetail = 'UPDATE `paymentdetails` SET `IsReturn`= 1 WHERE `ID`= '.$details[$i]['ChkDetailID']	;
									$this->m_dbConn->update($update_paymentDetail);									
								}
