	 ...
	 if($gid == 1)
	 ...
	 else if($gid == 3)
	  {
		  ...
		  else
		  {
			  	if($from <> "" && $to <> "")
				{
					$sql = "select ledgertbl.id, vouchertbl.Date,vouchertbl.RefNo , ledgertbl.ledger_name as Particular, incometbl.Debit, incometbl.Credit,VoucherID, ExternalCounter , incometbl.VoucherTypeID, acc.category_name as particular_category_name from `incomeregister` as incometbl JOIN `ledger` as ledgertbl on incometbl.LedgerID=ledgertbl.id JOIN `account_category` as acc ON acc.category_id = ledgertbl.categoryid JOIN voucher as vouchertbl ON incometbl.VoucherID = vouchertbl.id where incometbl.LedgerID='".$lid."' AND incometbl.Date BETWEEN '" .$from."' AND '".$to."' ORDER BY Date ASC";	  
				}
				else
				{
					$sql = "select ledgertbl.id, vouchertbl.Date,vouchertbl.RefNo , ledgertbl.ledger_name as Particular, incometbl.Debit, incometbl.Credit,VoucherID, ExternalCounter , incometbl.VoucherTypeID, acc.category_name as particular_category_name from `incomeregister` as incometbl JOIN `ledger` as ledgertbl on incometbl.LedgerID=ledgertbl.id JOIN `account_category` as acc ON acc.category_id = ledgertbl.categoryid JOIN voucher as vouchertbl ON incometbl.VoucherID = vouchertbl.id where incometbl.LedgerID='".$lid."' ";	  
...
{
		$Type = '';
		$sql2 = "select `id`,`Date`,VoucherNo, ExternalCounter, `VoucherTypeID`, `RefNo` from `voucher` where id='".$vid."' ";
...
public function getGroupMembers($type)
    {
		...
		else if($type == 6)		//Assets
		{
			...
			//$sql = "select led.id,led.ledger_name,led.categoryid ,acctbl.group_id,led.payment from `ledger` as led join `account_category` as acctbl  on led.categoryid =acctbl.category_id WHERE acctbl.group_id = '".ASSET."' AND led.categoryid <> '".DUE_FROM_MEMBERS."' ORDER BY acctbl.group_id,led.ledger_name ASC";
			$sql = "select led.id as LEDID,led.ledger_name as LEDNAME,led.categoryid ,acctbl.group_id,led.payment from `ledger` as led join `account_category` as acctbl  on led.categoryid =acctbl.category_id  WHERE acctbl.group_id = '".ASSET."' AND led.categoryid <> '".DUE_FROM_MEMBERS."'  ORDER BY acctbl.group_id,led.ledger_name ASC";
			$memId = $this->m_dbConn->select($sql);
		}	
		else if($type == 7)		//Income
		{
			...
			$sql = "select led.id as LEDID,led.ledger_name as LEDNAME,led.categoryid ,acctbl.group_id,led.payment from `ledger` as led join `account_category` as acctbl  on led.categoryid =acctbl.category_id  WHERE acctbl.group_id = '".INCOME."'   ORDER BY acctbl.group_id,led.ledger_name ASC";
			$memId = $this->m_dbConn->select($sql);
		}
		else if($type == 8)		//Expense
		{
			...
			$sql = "select led.id as LEDID,led.ledger_name as LEDNAME,led.categoryid ,acctbl.group_id,led.payment from `ledger` as led join `account_category` as acctbl  on led.categoryid =acctbl.category_id  WHERE acctbl.group_id = '".EXPENSE."'   ORDER BY acctbl.group_id,led.ledger_name ASC";
			$memId = $this->m_dbConn->select($sql);
		}
		//echo "Query sql : " . $sql;
