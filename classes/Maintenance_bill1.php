...
for($iVal = 0; $iVal < sizeof($BillRegisterData) ; $iVal++) 
{
	...
	if($BillDetails->sHeader != INTEREST_ON_PRINCIPLE_DUE)
	{
		...
		if($data[$iVal]["value"] <> '0.00' && $data[$iVal]["key"] <> ADJUSTMENT_CREDIT)
		{
			...
			else if($data[$iVal]["key"] <> INTEREST_ON_PRINCIPLE_DUE && ($data[$iVal]["key"] <>  SERVICE_TAX && $data[$iVal]["key"] <> IGST_SERVICE_TAX &&  $data[$iVal]["key"] <> CGST_SERVICE_TAX && $data[$iVal]["key"] <> SGST_SERVICE_TAX &&  $data[$iVal]["key"] <> CESS_SERVICE_TAX) )
			{
	    	?>
				<script> 
    ...
    <div style="border:1px solid black;">
        ...
        <div id="bill_details" style="text-align:center;border-top:1px solid black;font-size:<?php echo $BillFont?>px;">
			...
			{ ?>
                   	...
                   	<tr><td id='shareCertificate' style="width: 16%;">Share Certificate No : </td><td style="width: 16%;"> <?php echo $objFetchData->GetShareCertificateNo($_REQUEST["UnitID"]); ?></td></tr>	
                    				                
               <?php }
			   else{ ?>
                <!--<tr><td style="width: 7.5%">POS :  </td><td style="width: 16%;">MH (27)</td></tr>-->
               <?php }?>
        ...
        <td colspan="9">
        	<table  style="width:100%;font-size:14px;" id="mainTable">
                ...
                <tr>
                               ...
                               <table width="100%">
                               <tr>
                               <td style="border:1px solid black;text-align:left;font-size:<?php echo $BillFont?>px;padding-left:3px;font-weight: bold; width:56%">Bill No. : R-<?php echo sprintf('%05d', $BillNumber);?>/<?php echo $_SESSION['year_description']?></td>
...
<td style="border:1px solid black;text-align:right;font-size:<?php echo $BillFont?>px;padding-left:3px;font-weight: bold;">Due Date : <?php echo $DueDate ?></td>
...
<td colspan="3" style="border:1px solid black;text-align:right;font-size:<?php echo $BillFont?>px;padding-left:3px;font-weight: bold;">Due Date : <?php echo $DueDate ?></td>
				  ...
				  <td style="border-left: 1px solid black;border-right: 1px solid black;"><table style="width:100%"><tr><td style="border-right: 1px solid black;text-align:center;width:50%;font-weight:bold">Rate</td><td style="text-align:center;width:50%;font-weight:bold;border-right: 1px solid black;">Amount</td></tr></table></td>
						...
						if($data[$i]["key"] != ADJUSTMENT_CREDIT)
						{
							if($data[$i]["value"] <> 0)
							{
								...
								elseif(($data[$i]["key"] == SERVICE_TAX && $data[$i]["value"] >= 0) || ($data[$i]["key"] == IGST_SERVICE_TAX && $data[$i]["value"] >= 0 )|| ($data[$i]["key"] == SGST_SERVICE_TAX && $data[$i]["value"] >= 0 ) ||( $data[$i]["key"] == CGST_SERVICE_TAX && $data[$i]["value"] >= 0 )||( $data[$i]["key"] == CESS_SERVICE_TAX && $data[$i]["value"] >= 0))
								{
									$bIsParticular = false;
...
//		$gst_value = $m_objUtility->getRoundValue2(0.09 * $data[$i]["value"]);
						...
						if($data[$i]["value"] <> 0 && $data[$i]["key"] <> INTEREST_ON_PRINCIPLE_DUE && $data[$i]["key"] <> ADJUSTMENT_CREDIT && $data[$i]["key"] <> IGST_SERVICE_TAX && $data[$i]["key"] <> CGST_SERVICE_TAX && $data[$i]["key"] <> SGST_SERVICE_TAX && $data[$i]["key"] <> CESS_SERVICE_TAX)
						{
							$counter++;
						}
           ...
           <table style="width:100%;font-size:14px;">
                ...
                <tr>
                    ...
                    <td id="sub_total" style="text-align:right;width:16%;border:1px solid black;">
						<?php if($_SESSION['role']==ROLE_SUPER_ADMIN){ ?>
                        <input type="text" name="PrinciplePreviousArrears" id="PrinciplePreviousArrears" value="<?php echo ($PrinciplePreviousArrears); ?>" style="text-align:right;background-color:#FFFF00;" /> 
                        <?php } else { ?>
                ...
                <tr>
                    ...
                    <td id="sub_total" style="text-align:right;width:16%;border:1px solid black;">
						<?php if($_SESSION['role']==ROLE_SUPER_ADMIN){ ?>
                        <input type="text" name="" id="IntrestOnPreviousarrears" value="<?php echo ($IntrestOnPreviousarrears); ?>" style="text-align:right;background-color:#FFFF00;" />
                        <?php } else { ?>
