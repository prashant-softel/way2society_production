...
class BalanceSheet
{
	...
	function __construct($dbConn)
	{
		...
		$societyDetails = $this->m_objUtility->GetSocietyInformation($_SESSION['society_id']);
		$this->bill_format = $societyDetails['balancesheet_template'];
	}
	...
	public function LedgerArray($GroupID,$SubCategoryID, $from, $to,$IncomeExpenseAccount = 0,$YearID)
	{
		...
		if($GroupID == ASSET)
		{
			if($SubCategoryID == $_SESSION['default_cash_account'] || $SubCategoryID == $_SESSION['default_bank_account'])
			{
				...
				$sql = "SELECT ledgertbl.categoryid,ledgertbl.ledger_name as name,ledgertbl.id as LedgerID,SUM(bk.PaidAmount) as credit,SUM(bk.ReceivedAmount) as debit FROM `ledger` as ledgertbl LEFT JOIN account_category as actbl ON ledgertbl.categoryid = actbl.category_id LEFT JOIN `bankregister` as bk on ledgertbl.id = bk.LedgerID AND bk.Date BETWEEN '".$from."' AND '".$to."' where ledgertbl.society_id = ".$_SESSION['society_id']." AND ledgertbl.categoryid= ".$SubCategoryID." GROUP BY ledgertbl.id order by actbl.srno is null, actbl.srno, ledgertbl.srno is null, ledgertbl.srno";
		...
		if($GroupID == INCOME)
		{
			...
			$sqlPrev = "SELECT ledgertbl.categoryid,ledgertbl.ledger_name as name,ledgertbl.id as LedgerID,SUM(IF(incometbl.Credit IS NULL,'0.00',incometbl.Credit)) as creditPrev,SUM(IF(incometbl.Debit IS NULL,'0.00',incometbl.Debit)) as debitPrev FROM `ledger` as ledgertbl LEFT JOIN account_category as actbl ON ledgertbl.categoryid = actbl.category_id LEFT JOIN `incomeregister` as incometbl on ledgertbl.id = incometbl.LedgerID AND incometbl.Date BETWEEN '".date('Y-m-d', strtotime($from.' -1 year'))."' AND '".date('Y-m-d', strtotime($to.' -1 year'))."' where ledgertbl.society_id = ".$_SESSION['society_id']." AND ledgertbl.categoryid=".$SubCategoryID." GROUP BY ledgertbl.id order by actbl.srno, ledgertbl.srno";
		}
		...
		if($GroupID == EXPENSE)
		{
			...
			$sqlPrev = "SELECT ledgertbl.categoryid,ledgertbl.ledger_name as name,ledgertbl.id as LedgerID,SUM(IF(expensetbl.Credit IS NULL,'0.00',expensetbl.Credit)) as creditPrev,SUM(IF(expensetbl.Debit IS NULL,'0.00',expensetbl.Debit)) as debitPrev FROM `ledger` as ledgertbl LEFT JOIN account_category as actbl ON ledgertbl.categoryid = actbl.category_id LEFT JOIN `expenseregister` as expensetbl on ledgertbl.id = expensetbl.LedgerID AND expensetbl.Date BETWEEN '".date('Y-m-d', strtotime($from.' -1 year'))."' AND '".date('Y-m-d', strtotime($to.' -1 year'))."' where ledgertbl.society_id = ".$_SESSION['society_id']." AND ledgertbl.categoryid=".$SubCategoryID." GROUP BY ledgertbl.id order by actbl.srno, ledgertbl.srno";
		}	
				...
				if($netAmount <> 0 || $netAmount2 <> 0)
				{
					...
					else
					{
						...
						if($datas[$i]['LedgerID'] > 0)
						{
							...
							$Url = "view_ledger_details.php?lid=".$datas[$i]['LedgerID']."&gid=".$datas[$i]['GroupType']."&dt";
							$LedgetUrl = "ledger.php?edt=".$datas[$i]['LedgerID'];
							$tree .= '<tr id="li_' . $datas[$i]['id'] .'_'.$depth.  '"  style="display:none;" data-tt-parent-id="' . $datas[$i]['parent_id'] .'"  data-tt-id="-1"><td  style="width: 10px;" class = "icon-class"></td><td style="border:none;text-align:right;" class="Previous"><a href="'.$Url.'&prev" target="_blank">'.number_format($netAmount2,2).'</a></td><td style=" border:none;" class="icon-minus-sign">';	
	...
	public function getTotal($Data)
	{
		...
		foreach($Data as $key)
		{
			if($key['parent_id'] == 0 or $key['parent_id'] == 1)
			{
				...
				$total += ($this->bill_format == ABSOLUTE_BALANCESHEET) ? abs($tmp) : $tmp;
			}
		}	
	...
	public function FindAllChild($id ,&$Data,$unsetArray,$GroupID,&$DataShiftarray,$show = 0,$parentAmount = 0)
	{
		...
		for($i=0;$i < sizeof($Data);$i++)
		{
				...
				if($Data[$i]['parent_id'] == $id)
				{
						...
						{
							// In classic balance sheet times ledgers directly linked to current asset which we don't want to shift except due from members
							// To avoid such cases we have putted this condition 
							if($this->bill_format == CLASSIC_BALANCESHEET && $GroupID == ASSET && $Data[$i]['parent_id'] != DUE_FROM_MEMBERS){
								continue;
							}
		...
		for($i=0;$i < sizeof($Data);$i++)
		{
				...
				if($Data[$i]['parent_id'] == $id)
				{
						...
						if($amount < 0)
						{
							// In classic balance sheet times ledgers directly linked to current asset which we don't want to shift except due from members
							// To avoid such cases we have putted this condition 
							if($this->bill_format == CLASSIC_BALANCESHEET && $GroupID == ASSET && $id != DUE_FROM_MEMBERS){
