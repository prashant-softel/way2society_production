...
class add_member_id
{
	public $actionPage = "../add_member_id1.php";
	public $m_dbConn;
	public $m_dbConnRoot;
	...
	public function display1($rsas)
	{
			...
			$this->display_pg->edit="getadd_member_id1";
			$this->display_pg->th=$thheader;
			$this->display_pg->mainpg="add_member_id1.php";
			$res=$this->show($rsas);
			return $res;
	}
	...
	public function show($res)
	{
		if($res<>"")
		{
            ...
            <input type="button" id="SendAllTop" name="SendAll" onClick="SendAll()" style="text-align:center;float:right;display:none;margin: 1%;font-size: 14px;" value="Send Email to All Selected"/><b style="font-size:20px;;float:right;margin-top:5px">Invite All Units</b><input type="checkbox" id="checkall" name="checkall" onclick="ToggleCheckAll()" style="zoom:2;float:right"></input>
				...
				foreach($res as $k => $v)
				{
                    ...
                    <tr height="25" bgcolor="#BDD8F4">
						...
						else
						{
							...
							?>
                        		<td align="center"><?php if($memberProfile <> '') { echo $memberProfile[0]['owner_name']; }?></td>
                            <?php
						}
						?>
                        <td align="center"><?php echo $res[$k]['desc'];?></td>
                        <td align="center"><?php if($memberProfile <> '') { echo $memberProfile[0]['email']; } else { echo $memberDetails[0]['member_id']; }?></td>
