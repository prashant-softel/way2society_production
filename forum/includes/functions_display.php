...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	{
		//echo "test";
		$sql_array['JOIN'] = array(
			array(
				'FROM'	=> array(FORUMS_SOCIETYACCESS_TABLE => 'fsa'),
				'ON'	=> "fsa.forum_id = f.forum_id AND  fsa.society_id = '".$_SESSION['society_id']."' "
			)/*,
			array(
		...
		//$sql_array['FROM'][] = array(FORUMS_SOCIETYACCESS_TABLE => 'fsa' , FORUMS_TRACK_TABLE => 'ft');
		
		//$sql_where = "fsa.forum_id = f.forum_id AND  fsa.society_id =7  AND " . 'ft.user_id = ' . $user->data['user_id'] . ' AND ft.forum_id = f.forum_id';
		
		/*$sql_array['JOIN'][1] = array(
			'FROM' => array(FORUMS_TRACK_TABLE => 'ft'), 
			'ON' => 'ft.user_id = ' . $user->data['user_id'] . ' AND ft.forum_id = f.forum_id'
		);*/
