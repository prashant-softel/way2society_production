...
function get_society_list($forum_id = "" , $society_client_id)
{
	global $db, $user, $auth;

	// This query is identical to the jumpbox one
	$sql = "SELECT society_id, society_name
		FROM `society` where `status` = 'Y' and `client_id` = '".$society_client_id."' 
		ORDER BY society_id ASC";
	$result = $db->sql_query($sql, 600);
	$societyIDArray = array();
	if($forum_id <> "")
	{
		$sql_select = "SELECT `forum_id`,`society_id` FROM `phpbb_forums_societyaccess` where `forum_id` = '".$forum_id."' ";
		$result_select = $db->sql_query($sql_select, 600);
		
		$rows = $db->sql_fetchrowset($result_select);
		$m = 0;
		for($i= 0; $i < sizeof($rows); $i++)
		{
			if (!in_array($rows[$i]['society_id'], $societyIDArray)) 
			{
				$m++;
    			$societyIDArray[$m - 1] = $rows[$i]['society_id'];	
			}
			
			 		
		}
		//print_r($societyIDArray);
	}
	
	$padding_store = array('0' => '');
	$padding = '';
	$society_list = ($return_array) ? array() : '';

	
	$society_list .= '<option value= 0 >ALL</option>';
	while ($row = $db->sql_fetchrow($result))
	{
		if($forum_id <> '')
		{
			$selected = (in_array($row['society_id'], $societyIDArray)) ? ' selected="selected"' : '';
			$society_list .= '<option value="' . $row['society_id'] . '"' . $selected . '>' . $row['society_name'] . '</option>';
		}
		else
		{
			$society_list .= '<option value="' . $row['society_id'] . '" >' . $row['society_name'] . '</option>';		
		}
	}
	$db->sql_freeresult($result);
	return $society_list;
