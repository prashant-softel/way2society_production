			...
			switch ($action)
			{
				...
				case 'add':
					...
					$forum_data += array(
						...
						'forum_society_access'	=> request_var('forum_society_access',array(0)),
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				if ($action == 'edit')
				{
					...
					$society_list = get_society_list($row['forum_id'],CLIENT_ID);
				}
				else
				{
				...
				*/
				...
				$template_data = array(
					...
					'FORUM_SOCIETY_ACCESS'		=> $forum_data['forum_society_access'],
					...
					'S_FORUM_SOCIETY_ACCESS_OPTIONS' => $society_list,
					'L_FORUM_SOCIETY_ACCESS_NOTE'	=> "(Note : You can select multiple units by pressing ctrl key on keyboard + unit no.)"
				);
	...
	function update_forum_data(&$forum_data)
	{
		...
		*/
		...
		if ($is_new_forum)
		{
			...
			$society_array = $forum_data_sql['forum_society_access'];
			unset($forum_data_sql['forum_society_access']);
			...
			if(in_array('0' , $society_array))
			{
				
				//if all selected in society list
				$society_array = array();
				$sql_fetch = "SELECT society_id, society_name FROM `society` where `status` = 'Y' and `client_id` = '".CLIENT_ID."' ORDER BY society_id ASC";
				$result_fetch = $db->sql_query($sql_fetch, 600);
				$rows_fetch = $db->sql_fetchrowset($result_fetch);
				
				for($i= 0; $i < sizeof($rows_fetch); $i++)
				{
					$society_array[$i] = $rows_fetch[$i]['society_id'];	
				}
				
			}
			
			for($i = 0 ;$i < sizeof($society_array); $i++)
			{
				$query_society = "INSERT INTO `phpbb_forums_societyaccess`(forum_id, society_id) values ('$forum_id','".$society_array[$i]."')";
				//echo $query_society;
				$db->sql_query($query_society);
			}
		}
		else
		{
			...
			$society_array = $forum_data_sql['forum_society_access'];
			unset($forum_data_sql['forum_society_access']);
			$query_society_delete = "DELETE  from`phpbb_forums_societyaccess` where `forum_id` = '".$forum_id."'";
			//echo $query_society;
			$db->sql_query($query_society_delete);
			//var_dump($society_array);
