...
<script>
	function updateDuplicateVoucher()
	{
		var startNo = document.getElementById('start').value.trim();
		if(startNo.length > 0)
		{
			document.getElementById('msg').innerHTML = 'Message Log : <br />Fetching duplicate vouchers. Please Wait...';
			$.ajax({
				url : "script/UpdateDuplicateVoucherScript.php",
				type : "POST",
				data: { "start": startNo, 
					},
...
<body>
	...
	<p>NOTE : Select DB to check duplicate Vouchers.</p>
	<b>Start DB Name : </b>hostmjbt_society&nbsp;<input type="text" name="start" id="start" />
    <br><br>
    <input type="button" value="Update" onclick="updateDuplicateVoucher();" />
	<br /><br />
