<?php
	include_once('../../../classes/email_format.class.php');
	...
	{				
		...
		else
		{
			...
			{				
				$dueDateDetails = getResult($mMysqli, "SELECT `ReminderType`, `PeriodID`, `EventDate`, `ID` FROM `remindersms` WHERE `EventReminderDate` = '".date('Y-m-d')."' AND `society_id` = '".$data[$iCount]['society_id']."'  AND `CronJobTimestamp` = 0"); 				
	...
	{	
		...
		else
		{
			...
			$units = getResult($mMysqli1, "SELECT u.id, u.unit_no, mm.mob,mm.alt_mob FROM `unit` AS u JOIN `member_main` AS mm ON u.unit_id = mm.unit WHERE u.society_id = '".$societyID."'");						
			...
			for($i = 0; $i < sizeof($units); $i++)			
			{
				...
				{				
					...
					echo $objUtility->SendSMS($units[$i]['mob'],$mailBody);
					
					$msg = "<b>** INFORMATION ** </b>Unit - '".$units[$i]['unit_no']."' : Message Sent['".$sendSMS."']. <br /><br />";
					fwrite($Logfile,$msg);
