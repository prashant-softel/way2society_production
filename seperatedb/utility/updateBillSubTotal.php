...
<script>
	function updateBillSubTotal()
	{
		var dbNo = document.getElementById('start').value.trim();
		if(dbNo != '')
		{
			document.getElementById('msg').innerHTML = 'Message Log : <br />Updating Bill Subtotal . Please Wait...';
			$.ajax({
				url : "script/updateBillSubTotal.script.php",
				type : "POST",
				data: { "dbNo": dbNo, 
					},
...
<body>
	<h3>Select the Database to Update BillSubTotal For Mis Matched Entry</h3>

	<b>DB Name : </b>hostmjbt_society&nbsp;<input type="text" name="start" id="start" />
    <br><br>
    <input type="button" value="Update" onclick="updateBillSubTotal();" />
	<br /><br />
