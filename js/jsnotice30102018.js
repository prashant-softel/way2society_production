...
{	
	...
	{	
		...
		//This is for Send Sms to store original subject to show in sms text area
		document.getElementById("NoticeSubject").value = arResponse["subject"];
		...
		document.getElementById("mobile_notify").checked = (arResponse["mobile_notify"] == 1)? true : false;
		document.getElementById("insert").value = "Update";	
		document.getElementById("resend_notification").style.display = "inline-table";		
...
{
	var notice_id = document.getElementById('updaterowid').value;
	var sms_text = document.getElementById('SMSTemplate').value;
	var notification = {
		
		email: document.getElementById('notify').value,
		sms: document.getElementById('sms_notify').value,
		mobile: document.getElementById('mobile_notify').value
	...
	$.ajax
		({
			...
			type : "POST",
			datatype: "JSON",
			data : {"method":"resend_notifiction", "notice_id":notice_id, 'sms_text': sms_text, 'notification':notification},
			success : function(data)
			{	
				console.log(data);
