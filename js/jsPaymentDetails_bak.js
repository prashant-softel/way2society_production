...
function getExistingData(chqAry, LeafNo)
{
	$.ajax({
			url : "ajax/ajaxPaymentDetails.php",
			type : "POST",
			data: { "Cheque": JSON.stringify(chqAry), "LeafID":LeafNo, "mode":"Fill"} ,
			success : function(data)
			{	
				//alert(data);
				var arr1	= new Array();
...
function loadchanges()
{
	...
	if(arr1[0] == "edit")
	{
		...
		document.getElementById('ChequeDate').value=arr2[1];
		document.getElementById('ChequeNumber').value=arr2[2];
		document.getElementById('Amount').value=arr2[3];
		document.getElementById('PaidTo').value=arr2[4];
		document.getElementById('Timestamp').value=arr2[5];
		document.getElementById('EnteredBy').value=arr2[6];
		document.getElementById('PayerBank').value=arr2[9];
		document.getElementById('Comments').value=arr2[11];
		document.getElementById('VoucherDate').value=arr2[12];
	...
	else if(arr1[0] == "delete")
	{
		...
		window.location.href ="PaymentDetails.php?mst&"+arr1[1]+"&mm";
	}
	...
	for(var iRows = 1 ; iRows < iRowCount; iRows++)
	{
		...
		if(iSocietyID != "" && LeafID != "")
		{
			...
			if(iChequeNumber == "" || ChequeDate == "" ||  iAmount == "" || (iCheckState == 1 && ExpenseBy == 0) ||  (iCheckState == 1 && InvoiceDate2 == ""))
			{
				...
				if(iChequeNumber.length == 0 && LeafID == -1)
				{
					if(EmptyField.length == 0)
					{
