...
function Regenerate_Bill(UnitID,PeriodID,BillType)
{
	...
	//{
		...
		url:"ajax/ajaxgenbill.php",
		type:"POST",
		data:{"method":"regenerate_bill","UnitID":UnitID,"PeriodID":PeriodID,"BillType":BillType},
		success:function(data)
		{
			alert("Maintenance Bill Successfully Regenerated !!");
			location.reload(true);
			window.location.href = "Maintenance_bill.php?UnitID="+ UnitID+"&PeriodID="+PeriodID+"&BT="+BillType;
		}
		});	
	//}
}
		...
		for(var m=1;m <= NewRowCounter; m++)
		{
			...
			if(CheckSocietyApplyGST == 1 && RequestType == 'Invoice')
			{
				...
				if(LedgerTaxable == true)
				{
					...
					if(NoteType == 0)
					{
						LedgerTaxrate = document.getElementById('taxrate'+m).value;
					}
					else
					{
						LedgerTaxrate = 0;
			...
			if(HeaderID !=0 && CurrentValue !=0)
			{
				if(RequestType == 'Invoice')
				{
						...
						obj = {'Head' : HeaderID ,'Amt' : CurrentValue ,'HeadOldValue' : PreviousValue,'VoucherID' : VoucherID,'invoicetaxable':LedgerTaxable, 'TaxRate':LedgerTaxrate};
		...
		{	
			...
			$.ajax({
				...
				success : function(data)
				{
					...
					if($('#invoicetaxable'+Count).is(':checked') == false)
					{
						
						//document.getElementById('taxrate'+Count).disabled;
						...
						document.getElementById('taxrate'+Count).value=0;
					}
					else
					{
...
function GetInvoiceDetails(LedgerDetails,UserDetails,NoteType)
{
	CheckSocietyApplyGST = document.getElementById('SocietyTaxable').value;
	...
	{
		//console.log(LedgerDetails[i].TaxRate);
		...
		//console.log("SocietyApply",CheckSocietyApplyGST);
		if(CheckSocietyApplyGST == 1)
		{
			var TaxableChecked =document.getElementById('invoicetaxable'+RowCnt).checked = (LedgerDetails[i].Taxable != 0)? true : false ;	
			//console.log(LedgerDetails[i].Taxable);
			if(NoteType == 0)
			{
				document.getElementById('taxrate'+RowCnt).value=LedgerDetails[i].TaxRate;
				if(TaxableChecked == true && LedgerDetails[i].TaxRate == 0)
				{
					document.getElementById('taxrate'+RowCnt).value=18;
					$("#taxrate"+RowCnt).prop('disabled', false);
					document.getElementById('taxrate'+RowCnt).style.display='inline-block';
		 			document.getElementById('taxrateSet'+RowCnt).style.display='none';
				}
				else
				{
					...
					{
						$("#taxrate"+RowCnt).prop('disabled', true);
						document.getElementById('taxrate'+RowCnt).style.display='none';
		 				document.getElementById('taxrateSet'+RowCnt).style.display='inline-block';
					}
				}
			}
			//document.getElementById('taxrate'+RowCnt).value=LedgerDetails[i].TaxRate;	
