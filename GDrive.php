...
class GDrive
{
	...
	public function Initialize($bSetupProcess)
	{
		...
		{
			$curdir = "c:/wamp1/www/way2society-portal/conf/GoogleClientId.json";
			$curdir = "https://way2society.com/conf/GoogleClientId.json";
	...
	function insertFile($title, $description, $mimeType, $filename, $folderName, $folderDesc, $bUploadFile, &$parentFolderID, $bUpdateSocietyLevel = false)
	 {
		...
		//$objConn = new dbop($m_dbConn);
		echo "bUpdateSocietyLevel Data:".$bUpdateSocietyLevel;
		if($bUpdateSocietyLevel)
		{
			echo $sql = "update `society` set `GDrive_W2S_ID`='".$parentFolderID."'";
			//if($this->m_bShowTrace)
			{
				echo $sql;
			}
...
//print_r($createdFile);
	...
	function display_directory($List_of_documents, $first_call, $bGDriveTree, $bNotices = false, $DisplayType = 1)
	{
		...
		foreach($List_of_documents as $key => $value)
		{
			...
			if($Index == 0 && ($heading == "Notice" || $heading == "Misc" || $heading == "Lease" || $heading == "Lien"))
			{
			...
			else
			{
				...
				if($heading == "Misc" || $heading == "Lease" || $heading == "Lien")
				{
					//$link = "https://way2society.com/ViewNotice.php?id=".$value;	
					$sqlQry = "select name, attachment_gdrive_id,doc_version,Unit_Id,Document,TimeStamp from documents where doc_id='".$value ."'";
					//echo "heading:".$heading;
					if($heading == "Tenants" || $heading == "Lease")
					{
						$sqlQry = "select name, attachment_gdrive_id,doc_version,Document,Unit_Id,TimeStamp,refID from documents where refID='".$value ."'";
					}
					//echo "<br>".$sqlQry;
					$resultQry = $this->m_dbConn->select($sqlQry);
					if($heading == "Misc")
					...
					if($heading == "Lease")
					{
						...
						$link = "tenant.php?scm&id=" .$sMemberID ."&tik_id=". time()."&m&view=".$sRefID;
					}
					else
					{
					...
					else if($heading == "Lien")
					{
						//echo "In lien";
						//echo "value:".$value;
						$sqlLienQuery = "select * from mortgage_details where Id='".$value ."'";
						//echo "LienSql:".$sqlLienQuery;
						$resLienQuery = $this->m_dbConn->select($sqlLienQuery);
						/*echo "<pre>";
						print_r($resLienQuery);
						echo "</pre>";*/
						$startDate = $resLienQuery[0]["SocietyNOCDate"];
						$bankName = $resLienQuery[0]["BankName"]." Bank";
						$amt=$resLienQuery[0]['Amount'];
						$lienStatus=$resLienQuery[0]['LienStatus'];
						$closingDate= GetDisplayFormatDate($resLienQuery[0]['CloseDate']);
						$Title = GetDisplayFormatDate( $startDate) ." | ". $bankName . " | " . $amt ." | ".$lienStatus;
						if($lienStatus=="Closed")
						{
							$Title.=" | " . $closingDate;
						}
						$value = $Title;
						$lienLink = "<li class=\"pft-file " . strtolower($ext) . "\"><a title=\"$Title\" href=\"viewLien.php?lienId=".$resLienQuery[0]["Id"]."\" target=\"_blank\" style=\"cursor: pointer;\">" . htmlspecialchars($value) . "</a></li>";
					}
					else
					{
