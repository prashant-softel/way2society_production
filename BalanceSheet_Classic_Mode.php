...
include "classes/BalanceSheet_Classic_Mode.class.php";
...
<tr>
    ...
    <?php  
        ...
        if(!empty($societyDetails['Auth_Share_Capital_Text']) && !empty($societyDetails['Auth_Share_Capital_Amount'])) {
            echo '<ul><li><label><span style="border:none;width:270px;"><a href = "society.php?id='.$_SESSION['society_id'].'&show&imp" target = "_blank">'.$societyDetails['Auth_Share_Capital_Text'].'</a></span><span style="border:none;width:80px;text-align:right;"><a href = "society.php?id='.$_SESSION['society_id'].'&show&imp" target = "_blank">'.$societyDetails['Auth_Share_Capital_Amount'].'</a></span></label></li></ul>';
        }
        echo ($objLIABILITY->generateBalanceSheet($LIABILITYData,0,1));
