...
// Added New Coditionon show Per Month Exemption
$BillNoFor = $objFetchData->GetBillNo($_REQUEST["PeriodID"]);
$result = $objFetchData->GetSocietyDetails1($objFetchData->GetSocietyID($_REQUEST["UnitID"]));
$service_tax_threshold = $result[0]['service_tax_threshold'];
$service_tax_threshold = (float)($service_tax_threshold);

$bill_cycle = $result[0]['bill_cycle'];
$exeptionPerMonth =0;
if($bill_cycle == 1)
{
	$exeptionPerMonth=$service_tax_threshold/12 ;
}
elseif($bill_cycle == 2)
{
	$exeptionPerMonth=$service_tax_threshold/6;
}
elseif($bill_cycle == 3)
{
	$exeptionPerMonth=$service_tax_threshold/4;
}
elseif($bill_cycle == 4)
{
	$exeptionPerMonth=$service_tax_threshold/3;
}
elseif($bill_cycle == 5)
{
	$exeptionPerMonth=$service_tax_threshold/2;
}
else
{
	$exeptionPerMonth=$service_tax_threshold;
}
...
$show_member_email = $showInBillDetails[0]["show_mem_email"];
$specialChars = array('/','.', '*', '%', '&', ',', '(', ')', '"');
...
<body>
    ...
    <table style="width:100%;">
        ...
        <div style="float:left;">
			...
			{?>
        	  <img src="<?php echo "May23/".$objFetchData->objSocietyDetails->sSocietyLogo ?> " style="width:60px;margin-left: 10px;"></div>
			<?php 
			}
			else
			{?>
        	  <img src="SocietyLogo/main/default.png" style="width:60px;margin-left: 10px;"></div>
			<?php
        	}?>
			  </td>
        ...
        <div id="bill_details" style="text-align:center;border-top:1px solid black;font-size:<?php echo $BillFont?>px;">
            <table style="width:100%;">
            	<tr>
                    ...
                    <?php if($show_member_email)
					{?>
                    &nbsp;&nbsp;&nbsp;(<?php echo $objFetchData->objMemeberDetails->sEmail; ?> )
                    <?php }?>
                    </td>                    
            ...
            <table>
	            <tr>
	                ...
	                <td id='bill_no' style="width:15%;">
                    <?php if($IsSupplementaryBill == 0){?>
                    <b><?php echo $BillNoFor?>/<?php echo sprintf('%03d', $BillNumber)?></b>
                    <?php }
					else
					{?>
                     <b><?php echo $BillNoFor?>/RR/<?php echo sprintf('%03d', $BillNumber)?></b>
					<?php }?>
                    </td>
	            </tr>
				<tr>
        ...
        <div id="bill_charges">
        	<table  style="width:100%;font-size:14px;" id="mainTable">
                ...
                <?php
					...
					$bGSTParticularsThresholdAdded = false;
					$dNonGstLedgerTotal = 0;
					$dGstLedgerTotalTax = 0;
					$dGstLedgerThresholdTotalTax = 0;
					$gst_amount = 0;
					$dGstLedgerTotal = 0;
					...
					for($i=0 ;$i < sizeof($data);$i++)
					{
						...
						if($LedgerDetails[$data[$i]["key"]]['General']['taxable'] == 1)
						{
							...
							if($LedgerDetails[$data[$i]["key"]]['General']['taxable_no_threshold'] == 1)
							{
								array_push($taxable_no_threshold_ledgers,$data[$i]);	
								
							}
							elseif( $LedgerDetails[$data[$i]["key"]]['General']['taxable_no_threshold'] == 0)
							{
								array_push($taxable_ledger,$data[$i]);	
							}
							else
							{
					...
					{
						$gstTotal=$gstTotal+ $InterestOnArrears;
						//$InterestOnArrears+$TotalTaxableAmount;
					}
					...
					//echo "Amount ::".$gstTotal;
					//echo  " :: TotalTaxflag ::".$TotalTaxflag;
					$data = array_merge($non_taxable_ledgers,$taxable_ledger,$taxable_no_threshold_ledgers);
					...
					for($i=0; $i < sizeof($data) ;$i ++)
					{
						...
						if($bApplyServiceTax && $bGSTParticularsAdded == false  && $LedgerDetails[$data[$i]["key"]]['General']['taxable'] == 1)
						{
							...
							?>
								<tr>
				                	...
				                	<td colspan="3" style="border:1px solid black;text-align:left;font-size:<?php echo $BillFont?>px;padding-left:3px;font-weight: bold;">GST Charges With <?php echo $exeptionPerMonth ;?> Per Month Exemption</td>
				                	<td style="border:1px solid black;border-right:none;text-align:right;width:15%;font-size:<?php echo $BillFont?>px;"></td>
				                </tr>
						...
						if($bApplyServiceTax && $bGSTParticularsAdded == true && $bGSTParticularsThresholdAdded == false && $LedgerDetails[$data[$i]["key"]]['General']['taxable'] == 1 && $LedgerDetails[$data[$i]["key"]]['General']['taxable_no_threshold'] == 1)
							{
								
								if($bGSTParticularsAdded && !$bGSTParticularsThresholdAdded)
								{?>
								 <tr>
		                			<td style="border:1px solid black;border-left:none;text-align:center;font-size:<?php echo $BillFont?>px;font-weight: bold;"></td>
		                		<td colspan="3" style="border:1px solid black;text-align:right;font-size:<?php echo $BillFont?>px;padding-right:3px;font-weight: bold;">Sub Total of GST Charges With Exemption - B</td>
						...
						$bGSTParticularsThresholdAdded = true;
						?>
								<tr>
				                	<td style="border:1px solid black;border-left:none;text-align:center;font-size:<?php echo $BillFont?>px;font-weight: bold;">C</td>
				                	<td colspan="3" style="border:1px solid black;text-align:left;font-size:<?php echo $BillFont?>px;padding-left:3px;font-weight: bold;">GST Charges Without Exemption</td>
				                	<td style="border:1px solid black;border-right:none;text-align:right;width:15%;font-size:<?php echo $BillFont?>px;"></td>
				                </tr>
							<?php }
						...
						if($data[$i]["key"] != ADJUSTMENT_CREDIT)
						{
							if($data[$i]["value"] <> 0)
							{
								...
								if($bIsParticular == true)
								{
									...
									//echo "echo " .$bGSTParticularsThresholdAdded;
									else if($LedgerDetails[$data[$i]["key"]]['General']['taxable'] == 1 && $LedgerDetails[$data[$i]["key"]]['General']['taxable_no_threshold'] ==0)
									{
										$dGstLedgerTotal += $data[$i]["value"];
									}
									else
									{
										$dGstLedgerThresholdTotalTax += $data[$i]["value"];
									}
								}
							}
						}
					...
					{
						if(!$bGSTParticularsThresholdAdded)
						{
						?>
                        ...
                        <tr>
                         ...
                         <tr>
		                		<td style="border:1px solid black;border-left:none;text-align:center;font-size:<?php echo $BillFont?>px;font-weight: bold;"></td>
		                		<td colspan="3" style="border:1px solid black;text-align:right;font-size:<?php echo $BillFont?>px;padding-right:3px;font-weight: bold;">Sub Total of GST Charges Without Exemption - C</td>
		                		<td style="border:1px solid black;border-right:none;text-align:right;width:15%;font-size:<?php echo $BillFont?>px;font-weight: bold;"><?php echo number_format($dGstLedgerThresholdTotalTax, 2); ?></td>
		                </tr>
						<?php
						}
						else
           ...
           <?php
				...
				if($bApplyServiceTax && $bApplyGSTOnInterest)
				{
					$dGstLedgerThresholdTotalTax=$dGstLedgerThresholdTotalTax+ $InterestOnArrears;
				}
				
				if($TotalTaxflag)
				{
					$TotalTaxableAmount = $dGstLedgerTotal + $dGstLedgerThresholdTotalTax;
				}
				else
				{
					$TotalTaxableAmount = $dGstLedgerThresholdTotalTax;
				}
			?>
			...
			?>
          		<tr>
                	<!--<td colspan="3" rowspan="<?php echo $IsSupplementaryBill ? "2" : "7" ?>" style="width:<?php echo $IsSupplementaryBill ? "60%" : "50%" ?>">E.& O.E.</td>-->
					 ...
					 <td colspan="3" rowspan="<?php echo ($bApplyServiceTax == 1 || $BillTax > 0 || $CGST >0 || $SGST >  0 ) ? $value : 9 ?>" style="width:50%;padding-left:3px;font-size: 12px; border-top: 1px solid black;">
                    <label>E.& O.E.</label><br/><?php if($bApplyServiceTax == 1){?>Basic Exemption for GST is Rs.<?php echo $service_tax_threshold; }else {}?><br/><?php echo $sBillDetailNote;?>
                     </td>
                    	
                   
                    <td style="width:20%;border:1px solid black; border-top: 1px solid black;;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">Sub Total <?php echo $bApplyServiceTax ? "ST = (A + B + C)" : ""; ?></td>
               ...
               <?php if(isset($_REQUEST['edt'])){?>
				...
				<?php
					?>
					<tr>
		            	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">Interest on Arrears <?php echo (($bApplyServiceTax && $bApplyGSTOnInterest) ? "(D)" : "");  ?></td>
                    ...
                    <tr>
		            	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">Total Taxable Amount <?php echo (($bApplyServiceTax && $bApplyGSTOnInterest) ? "(E)" : "");  ?></td>
		                <td id="sub_total" style="text-align:right;width:20%;border:1px solid black;border-right:none;font-size:<?php echo $BillFont?>px;"><?php echo number_format($TotalTaxableAmount,2); ?></td>
		            </tr>
		        	<?php
		        }
				...
				//{
                    ...
                    {?>
	                <tr>
	                	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">SGST @ <?php echo $getSocietyInfo['sgst_tax_rate'] ?>% on E (F)<?php //echo ($bApplyGSTOnInterest ? " +  C" : "") ?></td>
					...
					{?>
                   <tr>
	                	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">CGST @ <?php echo $getSocietyInfo['cgst_tax_rate'] ?>% on E (G)<?php //echo ($bApplyGSTOnInterest ? " +  C" : "") ?></td>
					...
					{?>
                   <tr>
	                	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">IGST @ <?php echo $getSocietyInfo['igst_tax_rate'] ?>% on E (F)<?php //echo ($bApplyGSTOnInterest ? " +  C" : "") ?></td>
					...
					{?>
                   <tr>
	                	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">CESS @ <?php echo $getSocietyInfo['cess_tax_rate'] ?>% on E (G)<?php //echo ($bApplyGSTOnInterest ? " +  C" : "") ?></td>
                ...
                <td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">Total Outstanding Amount (ST + D + F + G)</td>
               <td id="sub_total" style="text-align:right;width:20%;border:1px solid black;border-right:none;font-size: <?php echo $BillFont?>px;font-weight: bold;"><?php echo number_format(abs($BalanceAmout), 2); if($BalanceAmout < 0){echo ' Cr';} else {echo ' Dr';} ?></td>
				...
				?><tr>
					...
					<?php 
									...
									{
										echo 'Total Outstanding Maintenance H= (ST + D + F + G)';
										
									}
									else if($IsSupplementaryBill == 1 )
									{
										if($_SESSION['society_id'] == 441)
										{
											echo 'Builder Dues (I)';
										}
										else
										{
											echo 'Total Outstanding Supplementry (I)';
										}
									}
									else if($IsSupplementaryBill == 3)
									{
										echo 'Total Outstanding Invoice (J)';
									}
                ...
                <tr>
					...
					<?php 
									if($IsSupplementaryBill == 0 )
									{
										...
										{
											echo 'Outstanding Supplementary Balance (I)';
										}
									...
									{
										echo 'Outstanding Maintenance Balance H= (ST + D + F + G)';
										/*if($_SESSION['society_id'] == 441)
										{
											echo 'Outstanding Maintenance Balance H= (ST + D + F + G)';
										}
										...
										{
											echo 'Outstanding Maintenance Balance (H)';
										}*/
				...
				<tr>
                	...
                	<td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">Outstanding Invoice Balance (J)</td>
			   ...
			   {?>
               <td style="width:20%;border:1px solid black;padding-left:3px;font-size:<?php echo $BillFont?>px;" colspan="2">Total Outstanding Balance (H + I + J) </td>
			   <?php }?> 
...
<?php } ?>
        ...
        <div id="bill_notes" style="text-align:left;font-size:<?php echo $BillFont?>px;margin-left:5px;">
        	...
        	<td >
               ...
               <div style="float:right;text-align:center"><b>Scan above QR code <br><span>to make the online payment</span></b></div>
        	</td>
