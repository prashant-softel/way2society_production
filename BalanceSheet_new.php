...
include "classes/BalanceSheet_20190603.class.php";
$parent=0;
...
$ExpenditureledgerDetails = $objLIABILITY->getIncomeExpenditureledgerDetails($_SESSION['default_income_expenditure_account']);
$ExpenditureledgerOpeningBalance = $ExpenditureledgerDetails[0]['OpeningBalance'];
$ExpenditureledgerAmt = $ExpenditureledgerDetails[0]['Amt'];
...
//This part written for manage the total when we stop the income and expenditure ledger to go on asset side 
if($ExpenditureledgerAmt <> 0 || $ExpenditureledgerOpeningBalance <> 0)
{
	if($ExpenditureledgerAmt <> 0)
	{
		$LIABILITYTotal += $ExpenditureledgerAmt;
	}
	
	if($ExpenditureledgerOpeningBalance <> 0)
	{
		$LIABILITYTotal += $ExpenditureledgerOpeningBalance;
	}
}
...
<div class="panel panel-info" id="panel" style="display:none;">
    <div class="panel-heading" id="pageheader">BALANCE SHEET NEW</div>
   <br />
...
<tr>
    ...
    <?php  
		...
		$IncomenExpenditureLedegerDetails = $objLIABILITY->getIncomeExpenditureledgerDetails($_SESSION['default_income_expenditure_account']);
		$IncomenExpenditureLedgerCatID = $IncomenExpenditureLedegerDetails[0]['CategoryID'];
		$IncomenExpenditureLedgerName = $IncomenExpenditureLedegerDetails[0]['LedgerName'];
		$IncomenExpenditureLedgerAmt = $IncomenExpenditureLedegerDetails[0]['Amt'];
		$IncomenExpenditureLedgerOpeningBalance = $IncomenExpenditureLedegerDetails[0]['OpeningBalance'];
		$AddorLess = $IncomenExpenditureLedegerDetails[0]['AddorLess'];
        
	   echo ('<div class="tree well">'.$objLIABILITY->generateBalanceSheet2($LIABILITYData,0,1));
		
		if($IncomenExpenditureLedgerAmt <> 0  || $IncomenExpenditureLedgerOpeningBalance <> 0)
		{
			
			if($IncomenExpenditureLedgerAmt <> 0)
			{
				$ProfitnLoss += $IncomenExpenditureLedgerAmt;	
			}
			
			if($IncomenExpenditureLedgerOpeningBalance <> 0)
			{
				$ProfitnLoss += $IncomenExpenditureLedgerOpeningBalance;
			}
		}
		
		if($IncomenExpenditureLedgerOpeningBalance <> 0)
		{
			$LastYearBalance += $IncomenExpenditureLedgerOpeningBalance;
		}
		$ProfitnLoss = abs($ProfitnLoss);
		
		echo '<ul><li><label><span style="border:none;width:270px;">Profit & Loss A/c</span><span style="border:none;width:50px;text-align:right;"><a href = "IncomeStmt.php" target = "_blank">'.number_format($ProfitnLoss,2).'</a></span></label><ul ><li style="display:none;"><label><span style="border:none;width:270px;font-weight:normal;" class="icon-minus-sign">Opening Balance</span><span style="border:none;width:50px;text-align:right;font-weight:normal;" class="icon-minus-sign">'.number_format($LastYearBalance,2).'</span></label></li></ul><ul ><li style="display:none;"><label><span style="border:none;width:270px;font-weight:normal;">Current Year</span><span style="border:none;width:50px;text-align:right;font-weight:normal;" class="icon-minus-sign">'.number_format($CurrentYearBalance,2).'</span></label></li></ul>';
		
		if($IncomenExpenditureLedgerAmt <> 0)
		{
			echo '<ul><li style="display:none;"><label><span style="border:none;width:270px;font-weight:normal;">'.$AddorLess.' : '.$IncomenExpenditureLedgerName.'</span><span style="border:none;width:50px;text-align:right;font-weight:normal;" class="icon-minus-sign">'.number_format($IncomenExpenditureLedgerAmt,2).'</span></label></li></ul></li></ul>';	
		}
		echo '</div>';
